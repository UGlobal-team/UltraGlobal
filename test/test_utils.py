...
def test_revisitop(cfg, ks, ranks):
    ...
    for i in range(len(gnd)):
        ...
        gnd_t.append(g)
    mapE, apsE, mprE, prsE, badE = compute_map(ranks_E, gnd_t, ks)
    ...
    for i in range(len(gnd)):
        ...
        gnd_t.append(g)
    mapM, apsM, mprM, prsM, badM = compute_map(ranks_M, gnd_t, ks)
    ...
    for i in range(len(gnd)):
        ...
        gnd_t.append(g)
    mapH, apsH, mprH, prsH, badH = compute_map(ranks_H, gnd_t, ks)

    pdb.set_trace()

    with open(r"D:\上海cv\SuperGlobal\revisitop\roxford5k\gnd_roxford5k.pkl", 'rb') as fin:
        gnd = pkl.load(fin)


    # for i in range(len(badE)):
    #     print(f"{gnd["qimlist"][badE[i]]}")
    #     logger.info(f"{gnd["qimlist"][badE[i]]}")

    # print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")

    # for i in range(len(badM)):
    #     print(f"{gnd["qimlist"][badM[i]]}")
    #     logger.info(f"{gnd["qimlist"][badM[i]]}")

    # print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")

    for i in range(len(badH)):
        print(f"{gnd["qimlist"][badH[i]]}")
        logger.info(f"{gnd["qimlist"][badH[i]]}")
